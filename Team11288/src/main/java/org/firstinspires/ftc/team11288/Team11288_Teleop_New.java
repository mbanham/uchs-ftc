...
@TeleOp(name="Team11288_Teleop_New", group="Teleop")
public class Team11288_Teleop_New extends OpMode{
  ...
  //  private DcMotor motorFrontRight;
   // private DcMotor motorFrontLeft;
    ...
    private double clawArmOffsetL = START_ARM_SERVO_L;
    private double clawArmOffsetR = START_ARM_SERVO_R;


    private final double CLAW_SPEED = 0.005;                 // sets rate to move servo
  ...
  //          leftArm.setPosition(START_ARM_SERVO_L);
    ...
    public void loop() {
       // teamUtils.printColorDistance(sensorColor, sensorDistance);
        ...
        if(gamepad1.dpad_up){
           teamUtils.driveFixedDistance(1.0, 2, false);
        }
        ...
        if(gamepad1.dpad_right){
            teamUtils.driveFixedDegrees(1.0,30.0);
        }
        ...
        if (gamepad2.dpad_up || gamepad2.dpad_down) {
            ...
            //teamUtils.runliftFixedDistance(directionLift,1.0);
            teamUtils.liftDrive(directionLift);
         }
         else{
        ...
        // Use gamepad2 left & right triggers to open the claw
        ...
        // Use gamepad2 left & right Bumpers to close the claw
        ...
        clawOffsetL = Range.clip(clawOffsetL, 0.65,0.86);
        clawOffsetR = Range.clip(clawOffsetR, 0.14, 0.35);
        ...
        //use just left arm for tilt of claw now
        //use right_stick_y >0.5
        if (gamepad2.right_stick_y > 0.5) {
            clawArmOffsetR -= CLAW_SPEED/2.0;
           // clawArmOffsetL += CLAW_SPEED;
        }
        ...
        // Move both servos to new position.  Assume servos are mirror image of each other.
       // clawArmOffsetL = Range.clip(clawArmOffsetL, 0.7,1.0);
        clawArmOffsetR = Range.clip(clawArmOffsetR, 0.1, 0.3);
       // leftArm.setPosition(clawArmOffsetL);
        rightArm.setPosition(clawArmOffsetR);
       // telemetry.addData("clawArmOffset L",  "= %.2f", clawArmOffsetL);
        telemetry.addData("clawArmOffset R",  "= %.2f", clawArmOffsetR);
